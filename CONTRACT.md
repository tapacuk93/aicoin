...
A service that wants to spend from a wallet has, until now, needed the owner to paste a token into it. That works and it is the wrong shape for a phone: the token is long, it is secret, and pasting it is the moment it gets into a screenshot. So a service can instead ask to be authorised and show a code.

`POST /wallet/api/authorize/new` — unauthenticated; asking commits nobody to anything. Body `{"service":"armedit","note":"..."}` → `{"id","secret","service","note","expires_in"}`. **The id and the secret are not interchangeable.** The id goes in the QR and is what the wallet sees; the secret stays with the service and is what the token is later collected with. If both were in the code, anyone who photographed the screen over somebody's shoulder could collect the token instead of the service that asked for it. Ten minutes, because an unanswered request is the ordinary outcome — somebody opened a page and walked away.

`GET /wallet/api/authorize/<id>` — unauthenticated. Without `?secret=` it answers what the *wallet* needs to decide: `{"status","service","note"}`. With it, and once granted, `{"status":"granted","token":"..."}` — after which the request is dropped. One collection only: a request that could be collected twice is a token that can be taken by whoever asks second.

`POST /wallet/api/authorize/<id>/approve` — **live-signed**, body `{"token":"<a token the wallet minted>"}`. Issuance stays exactly where it has always been: the wallet mints the token with its own private key and the proxy never sees that key. What the proxy checks is that the token verifies, that its address is the address that signed the approval, and that it names *this request* as its grant — a request id is a thing anybody who saw the screen has, and without those checks any of them could bind their own token to somebody else's request.

### Grants, and revoking one service
A token may carry `"grant":"<id>"` in its payload alongside `addr`, `iat` and `exp`. A grant is one service's permission to spend from one wallet, and it is a key in Redis that exists or does not: `aicoin:{aicoin}:grant:<address>:<id>`. **A token naming a grant is refused the moment that key is gone** (`401 "this authorisation has been revoked"`), and a token naming none behaves exactly as it always has.

That is the whole mechanism, and it is what makes per-service revocation possible at all: a token is self-verifying — the address inside it is the key that checks it — so nothing about the token itself can ever be withdrawn. What can be withdrawn is the wallet's permission for whoever holds it.

`GET /wallet/api/grants` (live-signed) → `{"grants":[{"id","service","granted"}]}` — what this wallet has authorised, so it can be taken back. The grants of a wallet are kept in a set as well as as keys, because "what have I authorised" is asked far more often than any one grant is checked and answering it by scanning the keyspace is how a ledger gets slow in the wrong place.

`POST /wallet/api/grants/revoke` (live-signed), body `{"id":"..."}` → `{"revoked":true,"id":"..."}`. Every token naming that grant stops working at once, which is the point: revoking is about the service, and the tokens it holds are not something the owner has a list of. A ledger that cannot be reached counts as revoked rather than as granted — that fails the call rather than the wallet.

**Revocation (all tokens)**: `POST /wallet/api/revoke-tokens` (live-signed, see above) sets `aicoin:token-revoked-before:{address}` to the current time in Redis. Verifying a token additionally checks `iat > revokedBefore` (missing key = never revoked). A token authenticates AI-proxy calls only — it **cannot** claim free coins or transfer coins, since those require the raw private key. A leaked token can run up AI-proxy usage against the wallet's balance until revoked/expired; it cannot drain coins via transfer.
...
**The panel** is every provider that (a) this proxy knows a chat shape for — `anthropic`, `openai`, `google`, `mistral`, `kimi` — and (b) this deployment has both a non-empty `apiKey` and a `consortium.models.<provider>` entry for, **ordered by what each is rated at for the request's subject** (see "Capability endpoints"; a caller may name the `subject`, otherwise it is tagged from the prompt and context). `elevenlabs` and `stability` are not chat APIs and are never on it; `cohere` is one but its shape is not implemented, so it is not either. A caller's `providers` list can only narrow that set, never extend it. An empty panel is `503`, not an empty answer.

The order is not cosmetic: the first panelist is the default **editor**, which merges every draft and writes every revision, and in `lead` mode writes the answer outright. Ranking by subject puts the strongest model for *this* question in that seat instead of whichever provider comes first in the config file. Nobody is dropped for a low rating — a panel is worth having because its members disagree, and a weaker model that spots something the editor missed has earned its turn. The rating decides who writes, not who is allowed to speak. The response reports the `subject` the panel was ranked on.
...
`POST /text`, `POST /image`, `POST /audio` — one request shape in, one response shape out, with the provider chosen by the proxy rather than named by the caller.

The rest of the proxy is provider-shaped on purpose: the client sends OpenAI's own request to OpenAI's own path with `X-AI: openai`. That is right for a client that has already chosen a provider and written to its API, and wrong for a client that just wants a paragraph written or a sentence spoken — it forces two decisions (which provider, and what that provider's body looks like this month) on callers who have no view on either.

Auth is the same API token as any paid call. Body:
```json
{
  "prompt": "...",          // required; max 32,000 chars (5,000 for /audio — longer is a document, not a clip)
  "context": "...",         // optional background, max 32,000 chars; also read by the subject tagger
  "subject": "code",        // optional; skips tagging. One of the nine subjects below
  "provider": "anthropic",  // optional; pins the provider. Never silently replaced by another
  "escalate": true,         // /text only; default true. False forces exactly one call
  "size": "1024x1024",      // /image only
  "voice": "..."            // /audio only; the provider's own voice id or name
}
```

**Subject tagging.** `SubjectTagger` tags the request as one of `code`, `math`, `translation`, `legal`, `science`, `creative`, `analysis`, `writing`, `general` from keyword signals in the prompt, falling back to the context only when the prompt matched nothing — a licensing question asked about a codebase is a licensing question, and any scheme that lets a pasted directory outvote the sentence with the question mark in it tags every request in a session the same way. Ties go to the more specific subject (the map is ordered specific-first, `writing` last). It is deliberately **not** a model call: asking a model what a request is about before answering it doubles the price of every short question to decide something that only *orders* a list. A wrong tag never refuses, never blocks and never fails a call — it just ranks differently, and that asymmetry is what makes cheap and approximate right here.

**Skill ratings.** `capabilities.skills.<provider>.<key>`, 0–5, where the key is a capability (`text`, `image`, `audio`), a subject (`code` — a rating for that provider's *text*), or a capability-prefixed subject (`image-creative`, `audio-creative`). The prefix matters: being rated 5 at writing code says nothing about who should read a sentence about code aloud, and an unprefixed rating decided exactly that until it was scoped. A capability rating of **0 means never route** — how "ElevenLabs writes no prose" and "Anthropic returns no images" are said. A subject rating overrides the capability rating for requests tagged with that subject; a subject with no entry means *no opinion*, not a bad score, and falls back to the capability rating. At equal scores an explicit subject rating outranks one reached by fallback, then `PROVIDER_NAMES` order breaks the tie so a request routes the same way every time it is asked. **These ratings are the operator's opinion, not a measurement** — nothing is benchmarked and nothing is learned from traffic. They are config for that reason, overridable per rating with `AICOIN_PROXY_SKILL_<PROVIDER>_<KEY>`.

**Choosing.** Candidates are the providers rated above 0 for the capability that this deployment has a key for, has a model for (`consortium.models` for text, `capabilities.<image|audio>.models` for media), whose shape an adapter knows, and that `GET /health` does not report as `down` — a rating is an opinion, a dead provider is a fact, and the fact wins. `unknown` (not probed yet) is not `down` and does not disqualify. The best one answers; if it fails outright the next takes it, up to 3 attempts. A failed call is refunded, so failover costs the caller nothing but the wait. A pinned `provider` is the only one tried: a caller who chose a provider did not ask to be quietly moved to a different one. No candidate is `503`; none answering is `502` with every error listed.

**Escalation (`/text` only).** The single model answering may reply with exactly `NEEDS CONSORTIUM` instead of an answer, and the same request is then put to the whole panel — drafted, merged, reviewed to a clean round. Whether one model is enough is a judgement that belongs to a model that has read the request, not to a rule about its length. But it spends the caller's coins several times over, so: the model is told what it costs and told that "large, open-ended or interesting" is not the bar; the phrase counts only as the entire reply (a reply that mentions it while answering is an answer); `escalate: false` opts out per request; and `capabilities.text.escalation: false` turns it off for the deployment. The escalated response is the consortium's, with `escalated: true`, `escalated_from: "text"`, and the triage call folded into `calls` and `coins_charged` — it was a real paid call, and a receipt that forgot it would not add up.

**Responses.** `/text` returns `{"answer", "escalated", "capability", "subject", "provider", "model", "considered", "calls", "coins_charged", "errors"}`; `/image` and `/audio` return the same routing fields with `{"media_base64", "media_type"}` in place of `answer`. Media always comes back as base64 in JSON, including from the providers that answer in raw bytes (ElevenLabs speech, and Stability unless asked for JSON). That costs a third in size and buys one response shape across providers, which is what makes the choice of provider genuinely the proxy's to make rather than something the caller branches on. `X-Aicoin-Charged` carries the total, as on every other paid call.

**Billing is not special.** Each attempt is an ordinary paid call: one coin held, the metered remainder settled from real cost, refunded if the provider never answered. Image and speech responses report no usable token count — an image model's token count has nothing to do with its price — so those are priced by the model's configured `usdPerCall`. Without that an image falls through to `defaultCostUsdPerCall` and a four-cent image is recorded as costing a tenth of a cent, which is then published as the coin price.

**`GET /skills`** — public and unauthenticated, like `/price` and `/health`: the rating table, and the live ranking each capability/subject pair would actually route by right now, filtered through keys and liveness. Routing a request on a caller's behalf is only defensible if the caller can find out how, and the table alone would not do it.
...
- `GET /skills` → the capability/subject rating table and the ranking each pair would route by right now (see "Capability endpoints"). Public and unauthenticated, `Access-Control-Allow-Origin: *`, like `/price` and `/health`.
- `GET /health` — for each configured provider (openai, anthropic, google, mistral, cohere, elevenlabs, stability, kimi), report whether recent upstream calls have hit rate-limiting or budget errors, and whether the proxy has a real (non-empty) `apiKey` configured for it at all (`enabled`) — this is what the landing page reads to show which AI backends are actually live. Track, per provider, a rolling window of the last `health.windowSize` forwarded calls (config, default 50, env `AICOIN_PROXY_HEALTH_WINDOW_SIZE`): `rateLimited` = true if any upstream response in the window was HTTP 429; `overBudget` = true if any was HTTP 402 or 403; `healthy` = `!rateLimited && !overBudget`. Response: `{"providers":[{"name":"openai","enabled":true,"healthy":true,"rateLimited":false,"overBudget":false,"state":"alive","detail":"responded 200","checkedAt":1757183940000,"latencyMs":180}, ...]}` (all providers always listed, even ones with zero calls so far — those default to `healthy:true`/`enabled` reflects config regardless of traffic). Always includes `Access-Control-Allow-Origin: *`, same as `/price`, since the landing page fetches it cross-origin too.

  **Liveness (`state`).** `healthy`/`rateLimited`/`overBudget` are inferences from calls this proxy happened to forward, so they say nothing about a provider nobody has called: a revoked key, an expired card or a dead host is indistinguishable from a healthy backend until the first customer call fails on it. So the proxy also **asks each provider directly**, on a timer: one `GET` per provider every `health.probe.intervalSeconds` (default 60, env `AICOIN_PROXY_HEALTH_PROBE_INTERVAL_SECONDS`), timing out after `health.probe.timeoutSeconds` (default 10), to that provider's `probePath` — a path in its own `freePaths`, so probing forever costs nothing — with the proxy's key injected exactly as a real call would inject it. Probing can be turned off entirely with `health.probe.enabled` / `AICOIN_PROXY_HEALTH_PROBE_ENABLED`, and any single provider's path can be overridden with `AICOIN_PROXY_<PROVIDER>_PROBE_PATH`.

  `state` is one of:
  - `alive` — the provider answered. A `429` counts as alive (it is up and throttling us, which is capacity, not an outage), as does any other unexpected non-error status, e.g. a `404` from a probe path this proxy has wrong: the provider answered, and this proxy's own misconfiguration must not be reported as the provider being down.
  - `down` — no answer before the timeout, a connection failure, a `5xx`, a `402` (reachable but out of credit), or a `401`/`403` (the key is rejected, so no call can be served however well the host is running).
  - `unconfigured` — no `apiKey` for this provider; it is never probed and never routed to.
  - `unknown` — configured but not yet probed (the proxy started less than one interval ago), or probing is disabled. **Never reported as `alive`**: "we have not looked" and "we looked and it answered" are different statements, and a status page that conflates them is worse than no status page.

  `detail` says what produced the state in words fit to print (`responded 200`, `rate limited`, `key rejected (401)`, `no response before timeout`), `checkedAt` is the probe's epoch-millis timestamp (`0` if never probed), and `latencyMs` is how long it took.
