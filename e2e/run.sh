...
log "--- test 33: a wallet's standing, for deciding whether to send to a stranger ---"
curl -s "http://127.0.0.1:$PROXY_PORT/wallet/api/reputation/$ADDR_CAROL" > "$WORKDIR/t33.json"
read -r rating calls owed <<<"$(python3 -c "
import json
d = json.load(open('$WORKDIR/t33.json'))
print(d['rating'], d['calls'], d['owed'])
")"
# Carol has claimed, made paid calls and dealt with others: a history, and nothing owed.
if python3 -c "import sys; sys.exit(0 if float('$owed') == 0 and int('$rating') >= 1 and int('$calls') >= 1 else 1)"; then
  pass "carol rates $rating with $calls call(s) and owes nothing"
else
  fail "expected a rating for a wallet with history, got rating=$rating calls=$calls owed=$owed"
fi

# A wallet nobody has ever dealt with rates 0 — not an accusation, and not an all-clear either.
KEY_STRANGER="$WORKDIR/stranger.pem"; gen_wallet "$KEY_STRANGER"; ADDR_STRANGER=$(wallet_address "$KEY_STRANGER")
curl -s "http://127.0.0.1:$PROXY_PORT/wallet/api/reputation/$ADDR_STRANGER" > "$WORKDIR/t33b.json"
stranger=$(python3 -c "
import json
d = json.load(open('$WORKDIR/t33b.json'))
print(d['rating'], any('never done anything' in r for r in d['reasons']))
")
